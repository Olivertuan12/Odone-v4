...
const STATUS_HEX: Record<string, string> = {
  'New Arrival': '#60a5fa', 
  'Editing': '#c084fc', 
  'Need Revision': '#fb923c', 
  'Need Improve': '#f87171', 
  'Approved': '#34d399', 
  'Delivered': '#2dd4bf' 
};

export const ProjectDetail = ({ projectId, onOpenDocument, onOpenVideo }: { projectId: string; onOpenDocument: (id: string) => void; onOpenVideo: (id: string) => void; }) => {
                   ...
                   <div className="space-y-1 overflow-y-auto custom-scrollbar flex-1 pr-1">
                     {documents.map(d => (
                       <button 
                         ...
                         className="w-full text-left group flex items-center gap-2 p-2 hover:bg-white/5 rounded transition-colors border border-transparent hover:border-white/5 bg-[#1A1A1C]"
                       >
                           ...
                           {videos.filter(v => (v.status || 'New Arrival') === status).map(v => (
                             ...
                             >
                                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-auto">
                                  <button onClick={() => onOpenVideo(v.id)} className="text-[9px] uppercase font-bold text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 px-1.5 py-0.5 rounded cursor-pointer" onMouseDown={(e) => e.stopPropagation()}>
                                    Open
                                  </button>
                                </div>
